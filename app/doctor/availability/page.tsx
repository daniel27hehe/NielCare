    ...
    <div className="min-h-screen" style={{ background: "#f0f1f5" }}>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 animate-fade-in">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold" style={{ color: "#1e40af" }}>Manage Availability</h1>
        ...
        <Card className="border-0 shadow-sm mb-6 rounded-3xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <span style={{ color: "#1e40af" }}>Add Schedule</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2"><Label>Slot Duration (min)</Label><Input type="number" value={newSchedule.slot_duration_minutes} onChange={e => setNewSchedule(p => ({ ...p, slot_duration_minutes: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Start Time</Label><Input type="time" value={newSchedule.start_time} onChange={e => setNewSchedule(p => ({ ...p, start_time: e.target.value }))} /></div>
              <div className="space-y-2"><Label>End Time</Label><Input type="time" value={newSchedule.end_time} onChange={e => setNewSchedule(p => ({ ...p, end_time: e.target.value }))} /></div>
            </div>
            <Button onClick={addSchedule} disabled={saving} className="rounded-xl text-white" style={{ background: "#1e40af" }}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Plus className="h-4 w-4 mr-2" />}Add Schedule
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-lg rounded-3xl bg-white">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <span style={{ color: "#1e40af" }}>Current Schedules</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {schedules.length === 0 ? (
              <p className="text-center text-slate-500 py-8">No schedules configured</p>
            ) : (
